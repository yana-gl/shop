...
function SidebarBasket({onCancelItem, onCancel, items = [], opened}) {
    ...
    const onClickOrder = async () => {
        try {
            const { data } = await axios.post('https://61f53c3462f1e300173c4065.mockapi.io/Orders', {items: basketItems});
            setOrderId(data.id);
            ...
            for (let i = 0; i < basketItems.length; i++) {
                ...
                await axios.delete('https://61f53c3462f1e300173c4065.mockapi.io/BasketItems/' + item.basketId);
                //await delay(1000);
            }
        } catch (error) {
